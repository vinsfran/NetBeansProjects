...
public class EstadosReclamosSB {
    ...
    public String crearEstadosReclamos(EstadosReclamos objeto) {
        ...
        try {
            em.persist(objeto);
            mensajes = objeto.getNombreEstadoReclamo() + " se creo con exito!";
        } catch (Exception ex) {
            mensajes = objeto.getNombreEstadoReclamo() + " no se pudo crear. (" + ex.getMessage() + ")";
        }
        return mensajes;
    }
    ...
    public String actualizarEstadosReclamos(EstadosReclamos objeto) {
        ...
        try {
            em.merge(objeto);
            mensajes = objeto.getNombreEstadoReclamo() + " se actualizó con exito!";
        } catch (Exception ex) {
            mensajes = objeto.getNombreEstadoReclamo() + " no se pudo actualizar. (" + ex.getMessage() + ")";
        }
        return mensajes;
    }
