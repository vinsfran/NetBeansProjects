...
import py.gov.mca.reclamosmca.entitys.Claims;
...
public class ClaimsSB {
    ...
    public String crearReclamos(Claims claims) {
        mensajes = "";
        try {
            if (claims.getDescripcionReclamoContribuyente() == null) {
                claims.setDescripcionReclamoContribuyente("NULL");
            }
            if (claims.getDescripcionReclamoContribuyente().equals("")) {
                claims.setDescripcionReclamoContribuyente("SIN DESCRIPCION");
            }
            if (claims.getFkImagen() != null) {
                Imagenes imagen = new Imagenes();
                imagen.setArchivoImagen(ajustarImagen(claims.getFkImagen().getArchivoImagen(), claims.getFkImagen().getTipoImagen()));
                imagen.setNombreImagen(claims.getFkImagen().getNombreImagen());
                imagen.setTipoImagen(claims.getFkImagen().getTipoImagen());
                em.persist(imagen);
                claims.setFkImagen(imagen);
                //  em.persist(objeto.getFkCodPersona());
            }
            em.persist(claims);
            em.flush();

            TiposReclamos tipoReclamo = tiposReclamosSB.consultarTipoReclamo(claims.getFkCodTipoReclamo().getCodTipoReclamo());
            tipoReclamo.setTopTipoReclamo(tipoReclamo.getTopTipoReclamo() + 1);

            em.merge(tipoReclamo);

            if (claims.getEnviarMail()) {
                String asunto = "RESPUESTA SOBRE RECLAMO " + claims.getFkCodTipoReclamo().getNombreTipoReclamo();
                String mensaje = "<html>"
                        + "     <head>"
                        + "         <meta charset=\"UTF-8\">"
                        ...
                        + "       <div> "
                        + "             <p>"
                        + "                Estimado/a: <i>" + claims.getLogin() + "</i>"
                        + "             </p> "
                        + "             <p>"
                        + "                Le informamos que su reclamo sobre "
                        + "                <strong>" + claims.getFkCodTipoReclamo().getNombreTipoReclamo() + "</strong>, "
                        + "                realizado en fecha "
                        + "                <strong>" + formatearFecha(claims.getFechaReclamo()) + "</strong> "
                        + "                fue ingresado a nuestro sistema de reclamos, se encuentra en la "
                        + "                <strong>" + claims.getFkCodTipoReclamo().getFkCodDependencia().getNombreDependencia() + "</strong>, "
                ...
                enviarCorreos.enviarMail(claims.getLogin(), asunto, mensaje);
            }
            //        tx.commit();
            mensajes = "OK";
        } catch (Exception ex) {
    ...
    public String actualizarReclamos(Claims claims) {
        mensajes = "";
        try {
        ...
        if (mensajes.equals("OK")) {
            if (claims.getEnviarMail()) {
                ...
                String mensaje = "<html>"
                        ...
                        + "                         <td>" + formatearFecha(claims.getFechaReclamo()) + "</td>"
                        + "                     </tr>"
                        + "                     <tr>"
                        + "                         <td><strong>Nro. de reclamo:</strong></td>"
                        + "                         <td>" + claims.getCodReclamo() + "</td>"
                        + "                     </tr>"
                        + "                     <tr>"
                        + "                         <td><strong>Tipo de reclamo:</strong></td>"
                        + "                         <td>" + claims.getFkCodTipoReclamo().getNombreTipoReclamo() + "</td>"
                        + "                     </tr>"
                        + "                     <tr>"
                        + "                         <td><strong>A cargo de:</strong></td>"
                        + "                         <td>" + claims.getFkCodTipoReclamo().getFkCodDependencia().getNombreDependencia() + "</td>"
                        + "                     </tr>"
                        + "                     <tr>"
                        + "                         <td><strong>Dirección de reclamo:</strong></td>"
                        + "                         <td>" + claims.getDireccionReclamo() + "</td>"
                        + "                     </tr>"
                        + "                     <tr>"
                        + "                         <td><strong>Detalle adicional del reclamo:</strong></td>"
                        + "                         <td>" + claims.getDescripcionReclamoContribuyente() + "</td>"
                        + "                     </tr>"
                        + "                     <tr>"
                        ...
                        + "                         <td>" + formatearFecha(claims.getFechaAtencionReclamo()) + "</td>"
                        + "                     </tr>"
                        + "                     <tr>"
                        + "                         <td><strong>Recepcionado por:</strong></td>"
                        + "                         <td>" + usuariosSB.consultarUsuariosPorLogin(claims.getUsuarioAtencion()).getFkCodPersona().getNombrePersona() + " " + usuariosSB.consultarUsuariosPorLogin(claims.getUsuarioAtencion()).getFkCodPersona().getApellidoPersona() + " </td>"
                        + "                     </tr>"
                        + "                     <tr>"
                        + "                         <td><strong>Tarea a realizar:</strong></td>"
                        + "                         <td>" + claims.getDescripcionAtencionReclamo() + " </td>"
                        + "                     </tr>";

                if (claims.getFkCodEstadoReclamo().getCodEstadoReclamo().equals(3)) {
                    mensaje = mensaje + "<tr>"
                            + "             <td colspan='2' style='text-align: center'><strong>DATOS DE FINALIZACIÓN.</strong></td>"
                            + "         </tr>"
                            + "         <tr>"
                            + "             <td><strong>Fecha de finalización:</strong></td>"
                            + "             <td>" + formatearFecha(claims.getFechaCulminacionReclamo()) + "</td>"
                            + "         </tr>"
                            + "         <tr>"
                            + "             <td><strong>Finalizado por:</strong></td>"
                            + "             <td>" + usuariosSB.consultarUsuariosPorLogin(claims.getUsuarioCulminacion()).getFkCodPersona().getNombrePersona() + " " + usuariosSB.consultarUsuariosPorLogin(claims.getUsuarioCulminacion()).getFkCodPersona().getApellidoPersona() + "</td>"
                            + "         </tr>"
                            + "         <tr>"
                            + "             <td><strong>Cantidad de dias en el sistema:</strong></td>"
                            + "             <td>" + claims.getCantidadDiasProceso() + "</td>"
                            + "         </tr>"
                            + "         <tr>"
                            + "             <td><strong>Detalle del trabajo realizado:</strong></td>"
                            + "             <td>" + claims.getDescripcionCulminacionReclamo() + "</td>"
                            + "         </tr>";
                }
